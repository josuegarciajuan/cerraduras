...
// loop() envía la telemetría con el cliente TLS compartido.
    ...
    if (event.ascii == '\r' || event.ascii == '\n') {
      if (scanned.length() > 0) {
        ...
        if (dots != 2 || scanned.length() < QR_MIN_LEN) {
          ...
          // FIX #1: encolar telemetría (sin HTTP aquí). Sin relayClick(): el
          // relé solo se acciona por apertura real.
...
void loop() {
  ...
  // ── FIX #1: telemetría de rechazo QR diferida (enviada desde loop()) ──
  if (qrRejectPending) {
