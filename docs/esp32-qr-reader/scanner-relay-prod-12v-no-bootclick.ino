/**
 * scanner-relay-prod-12v-no-bootclick.ino — Producción QR + Relé 12V (variante SIN
 * clic de relé en arranque). Basado en scanner-relay-prod-12v.ino con 2 cambios:
 *   1) wifiConnectedFeedback() ya NO hace relayClick() al conectar WiFi.
 *   2) Las lecturas QR RECHAZADAS ya NO hacen relayClick().
 * MOTIVO: con la cerradura (solenoide) conectada al relé, el pulso de arranque
 * de 150 ms disparaba el pestillo en el boot y el pico de corriente hundía la
 * fuente (brownout) → reinicio en bucle ~10 s sin llegar al heartbeat. El relé
 * solo se acciona por apertura real (relayPulse) o verificación manual (check).
 * Resto de características idénticas al original (WiFi NVS, WiFiManager,
 * auto-wipe por fingerprint, heartbeat 30 s, F33 command-queue, announce F40).
 *
