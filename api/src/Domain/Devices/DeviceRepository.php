 ...
 *
 * Devices are now primarily resolved through packs:
 *   room → room.pack_id → devices.pack_id
 * Falls back to direct devices.room_id for backward compat.
 */
final class DeviceRepository implements DeviceRepositoryInterface
{
    ...
    public function listForRoom(int $roomId): array
    {
        $stmt = $this->pdo->prepare(
            'SELECT d.id, d.room_id, d.pack_id, d.kind, d.external_id, d.api_client_id, d.meta_json, d.is_identified, d.identified_at
             FROM devices d
    ...
    {
        $stmt = $this->pdo->prepare(
            'SELECT id, room_id, pack_id, kind, external_id, label, api_client_id, meta_json, is_identified, identified_at
    ...
    {
        $stmt = $this->pdo->query(
            'SELECT d.room_id, r.code, d.id AS device_id, d.external_id, d.identified_at
             FROM devices d
    ...
    private function hydrate(array $row): Device
    {
        ...
        return new Device(
            (int) $row['id'],
            isset($row['room_id']) && $row['room_id'] !== null ? (int) $row['room_id'] : null,
